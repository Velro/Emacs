...
(defun casey-big-fun-c-hook ()
  ...
  (define-key c++-mode-map "\e1" 'imenu)
...
(define-key my-mode-map "\M-l" 'forward-char)
(define-key my-mode-map "\M-j" 'backward-char)
(define-key my-mode-map "\M-i" 'previous-line)
(define-key my-mode-map "\M-k" 'next-line)

(define-key my-mode-map (kbd "C-M-l") 'forward-word)
(define-key my-mode-map (kbd "C-M-j") 'backward-word)
...
(define-key global-map "\e9" 'imenu)
...
(define-key global-map "\e2" 'casey-replace-in-region)
...
(define-key global-map "\e4" 'kill-this-buffer)
...
(define-key global-map [f5] 'unity-recompile-game)
